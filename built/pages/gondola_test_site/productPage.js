...
let productPage = class productPage {
    ...
    async addToCart(productName, cartCount = 1) {
        ...
        await gondola_1.gondola.waitForElement(realLocator);
        await gondola_1.gondola.click(realLocator);
        await gondola_1.gondola.waitForElementProperty(this.lblCartCount, "innerText", cartCount.toString(), 5);
    }
    async checkSalePageTitleDisplayed() {
        await gondola_1.gondola.checkControlExist(this.lblPageTitle);
        await gondola_1.gondola.checkControlProperty(this.lblPageTitle, "innerText", "Sale");
    }
    async checkAllSaleLabelsDisplayed() {
        let count = await gondola_1.gondola.getElementCount(this.elmItems);
        while (--count > 0) {
            await gondola_1.gondola.checkControlExist(`${this.elmItems}[${count}]//span[text()='Sale']`);
        }
    }
    async openShoppingCart() {
        await gondola_1.gondola.waitForElement(this.lnkTopMenuShoppingCart);
        await gondola_1.gondola.click(this.lnkTopMenuShoppingCart);
    }
};
__decorate([
    gondola_1.locator
...
__decorate([
    gondola_1.action("add item to cart", "Add product to cart")
], productPage.prototype, "addToCart", null);
__decorate([
    gondola_1.action("check page title displayed", "Check 'Sale' page title is displayed")
], productPage.prototype, "checkSalePageTitleDisplayed", null);
__decorate([
    gondola_1.action("check all sale labels exist", "Check 'Sale' label displayed in all products")
], productPage.prototype, "checkAllSaleLabelsDisplayed", null);
__decorate([
    gondola_1.action("open shopping cart", "Open shopping cart by clicking on cart menu on the top")
